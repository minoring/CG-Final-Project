...
class Camera {
   ...
   public:
    Camera()
        : position_(0, 0, 0.5f),
          ...
          fovy_(45),
          mode_(kOrtho) {}
    Camera(const vec3& _position, const vec3& _front_dir, const vec3& _up_dir,
           float _fovy)
        : position_(_position),
          front_dir_(_front_dir),
          up_dir_(_up_dir),
          fovy_(_fovy) {
   ...
   private:
    ...
    vec3 front_dir_;  // front direction of the camera    (it should be a unit
                      // vector whose length is 1)
    vec3 up_dir_;     // up direction of the camera       (it should be a unit
                      // vector whose length is 1)
    vec3 right_dir_;  // right direction of the camera    (it should be a unit
                      // vector whose length is 1)
