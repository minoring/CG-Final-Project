...
class mat {
   public:
    ...
    mat(const T elem) { std::fill(val, val + M * N, elem); }
    ...
    void set_to_zero() { std::fill(val, val + M * N, static_cast<T>(0)); }

    void set_to_identity() {
        assert(M == N);

        set_to_zero();
        for (unsigned int i = 0; i < M; ++i) (*this)(i, i) = 1;
    }
    ...
    void get_ith_row(unsigned int i, vec<N, T>& row) const {
        for (unsigned int c = 0; c < N; ++c) row(c) = (*this)(i, c);
    }

    void set_ith_row(unsigned int i, const vec<N, T>& row) {
        for (unsigned int c = 0; c < N; ++c) (*this)(i, c) = row(c);
    }
