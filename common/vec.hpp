...
class vec {
   public:
    ...
    vec(const T elem) { std::fill(val, val + N, elem); }
    ...
    vec(const vec<N, T>& other) { std::copy(other.val, other.val + N, val); }
    ...
    vec& operator+=(const vec<N, T>& other) {
        for (unsigned int i = 0; i < N; ++i) val[i] += other.val[i];
    ...
    void set_to_zero() { std::fill(val, val + N, static_cast<T>(0)); }
